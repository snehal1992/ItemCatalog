...
# IMPORTS FOR authentication flow
...
# categories = [{'name': 'The CRUDdy Crab', 'id': '1'}, {'name':'Blue Burgers', 'id':'2'},
#{'name':'Taco Hut', 'id':'3'}]
...
def gconnect():
    ...
    #see if users exists, if not make a user
...
# Route to show latest items and categories


@app.route('/')
@app.route('/category', methods=['GET'])
def showCategoryAndLatestItems():
    category = session.query(Category).all()
    item = session.query(Item).all()
    return render_template('categorylist.html', category=category, item=item)

# Route to show items based on category


@app.route('/category/<int:category_id>/', methods=['GET'])
def showSelectedCategoryList(category_id):
...
def newItem():
    ...
    if request.method == 'POST':
    	...
    	categoryResult = session.query(Category).filter_by(name=categoryName).one()
...
def editItem(category_id, item_id):
    ...
    if request.method == 'POST':
        ...
        print itemToEdit.name
        session.add(itemToEdit)
        session.commit()
        return redirect(url_for('showCategoryAndLatestItems'))
    else:
...
# Route for showing selected category  items  function
...
def showItem(category_id, item_id):
    ...
    itemAll = session.query(Item).filter_by(id=item_id)
    if 'username' not in login_session :
        return render_template('publicitem.html', item = itemAll,
             creator = user_id)
    else:
        #return render_template('publicitem.html', items = items,
             #creator = creator)
        return render_template('show-item.html', item=item, creator = user_id)


if __name__ == '__main__':
    app.secret_key = 'super_secret_ke'
    app.debug = True
